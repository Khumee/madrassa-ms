...
app.get('/attendance/students/:classId', hasRole(['ناظم', 'مدير', 'عریب']), async (req, res) => {
    ...
    const date = req.query.date || DateTime.now().toISODate();
        ...
        `, [teacher_id, day_of_week, period_number, end_time || '23:59', start_time || '00:00']);
            ...
            WHERE s.is_active = TRUE
        `);
                    res.render('periods_manage', { periods, teachers, classes, assignments, groupBy, generateSuccess: req.query.generateSuccess });
                } catch (err) {
                    console.error(err);
        ...
        `);
            ...
            app.post('/periods/edit/:id', hasRole(['ناظم', 'مدير', 'عريب']), async (req, res) => {
                const { assignmentId, dayOfWeek, periodNumber, startTime, endTime } = req.body;
                try {
                    ...
                    if (assignment.length > 0) {
                        await db.execute(
                            'UPDATE periods SET assignment_id = ?, day_of_week = ?, period_number = ?, start_time = ?, end_time = ?, teacher_id = ?, class_id = ? WHERE id = ?',
                            [assignmentId, dayOfWeek, periodNumber, startTime, endTime, assignment[0].teacher_id, assignment[0].class_id, req.params.id]
                        );
                    }
                    res.redirect('/periods/manage');
                } catch (err) {
