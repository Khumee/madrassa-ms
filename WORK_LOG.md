...
## 🐳 7. Transient Sandbox Testing Environment & Seeding Architecture

To support a seamless, conflict-free local development and evaluation environment, we decoupled the database lifecycle into structural migrations and programmatic seeding scripts, wrapping them in an automated, transient Docker infrastructure:

### A. Decoupled Seeding Design
* **Pure DDL Versioning**: The Flyway-compatible migrations (e.g., `V1__Schema.sql`) only declare database schema structures and constraint changes. All lookup codes and tenant datasets are completely isolated from schema migrations.
* **Stateless Programmatic Seeders**: Initialized a base tenant schema configuration via `seed.js` and rich mock user activities via `seed_demo_urdu.js`.
* **Super-Admin AJAX Seeder Trigger**: Integrated an endpoint `POST /admin/tenants/seed/:id` in `superAdminController.js`. Admins can dynamically seed realistic student, teacher, and attendance progress data for any tenant directly from the Urdu control panel.

### B. Transient "Build, Use, and Destroy" Lifecycle
* **Host Port Isolation**: Mapped standard MySQL and Node container ports to custom external host ports (`3308` and `3001` respectively) to prevent collisions with native developer servers.
* **Volume Exclusion**: Deliberately omitted persistent storage volumes for the database in the sandbox `docker-compose.yml` to guarantee a clean, side-effect-free test run every time the containers are initialized.
* **Automated Startup Sequence (`docker-start.sh`)**: Crafted an orchestration shell script that acts as the container entrypoint, sequentially executing:
  1. MySQL connection readiness check.
  2. Baseline Flyway schema migrations.
  3. Base tenant and demo database seeds.
  4. Node.js Express server process execution.
