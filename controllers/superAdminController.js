...
const { seedDemoForTenant } = require('../scripts/seed_demo_urdu');
...
exports.createTenant = async (req, res) => {
    ...
    try {
        ...
        const logoUrl = req.file ? `/uploads/logos/${req.file.filename}` : '/images/default_logo.png';

        // Insert
        const [result] = await db.pool.execute(
            `INSERT INTO tenants (name, subdomain, custom_domain, school_name, plan_tier, max_students, max_teachers, max_classes, logo_url) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                name, 
...
exports.seedTenant = async (req, res) => {
    const { id } = req.params;
    try {
        console.log(`Manually triggering demo seeding for tenant ID: ${id}`);
        await seedDemoForTenant(parseInt(id));
        res.json({ success: true });
    } catch (err) {
        console.error('Error seeding tenant via controller:', err);
        res.status(500).json({ success: false, error: err.message });
    }
};
